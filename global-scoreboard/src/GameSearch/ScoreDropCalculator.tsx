  ...
  const onCalculate = () => {
    ...
    getRunDetails(runId).then(run =>
      getLeaderboardRuns(run.game, run.category, run.values).then(records => {
        ...
        const m = primaryTimes.reduce((previous, current) => previous + current, 0) / primaryTimes.length
  ...
  return <div style={{ marginTop: '1rem', marginBottom: '1rem' }}>
    ...
    {requiredTime &&
      <span>
        ...
        <strong> {new Date(requiredTime * 1000).toISOString().substr(11, 8)}</strong> or less.
      </span>}
  </div>
}
